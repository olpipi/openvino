...
void fill_data_with_broadcast(InferenceEngine::Blob::Ptr& blob, size_t axis, std::vector<float> values) {
    SizeVector value_dims(blob->getTensorDesc().getDims().size() - axis, 1);
...
InferenceEngine::Blob::Ptr make_reshape_view(const InferenceEngine::Blob::Ptr &blob, SizeVector new_shape) {
