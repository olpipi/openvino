...
void GRUSequenceTestNew::SetUp() {
    ...
    std::vector<ov::Shape> inputShapes = {{{batch, seq_lengths, input_size},
    ...
    if (m_mode == SequenceTestsMode::CONVERT_TO_TI_MAX_SEQ_LEN_PARAM ||
        ...
        seq_lengths_node = param;
               ...
               m_mode == SequenceTestsMode::PURE_SEQ_RAND_SEQ_LEN_CONST) {
        auto tensor = ov::test::utils::create_and_fill_tensor(ov::element::i64, inputShapes[2], seq_lengths, 0);
        seq_lengths_node = std::make_shared<ov::op::v0::Constant>(tensor);
    } else {
        std::vector<int64_t> lengths(batch, seq_lengths);
        seq_lengths_node = std::make_shared<ov::op::v0::Constant>(ov::element::i64, inputShapes[2], lengths);
    }
...
void GRUSequenceTestNew::generate_inputs(const std::vector<ov::Shape>& targetInputStaticShapes) {
    ...
    for (const auto &param : function->get_parameters()) {
        auto tensor = ov::test::utils::create_and_fill_tensor(param->get_element_type(), *itTargetShape++, m_max_seq_len, 0);
