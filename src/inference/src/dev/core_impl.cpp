...
ov::SoPtr<ov::ICompiledModel> ov::CoreImpl::load_model_from_cache(
    ...
    try {
        cacheContent.cacheManager->read_cache_entry(
            ...
            [&](std::istream& networkStream, std::shared_ptr<ov::AlignedBuffer> mmap_buffer) {
                ...
                if (mmap_buffer) {
                    compiled_model = context ? plugin.import_model(networkStream, mmap_buffer, context, update_config)
                                             : plugin.import_model(networkStream, mmap_buffer, update_config);
                } else {
