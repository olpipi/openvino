...
ov::SoPtr<ov::ICompiledModel> ov::Plugin::import_model(std::istream& model, std::shared_ptr<ov::AlignedBuffer> mmap_buffer, const ov::AnyMap& properties) const {
    OV_PLUGIN_CALL_STATEMENT(return {m_ptr->import_model(model, mmap_buffer, properties), m_so});
}
...
ov::SoPtr<ov::ICompiledModel> ov::Plugin::import_model(std::istream& model,
                                                       ...
                                                       const ov::SoPtr<ov::IRemoteContext>& context,
                                                       const ov::AnyMap& config) const {
    OV_PLUGIN_CALL_STATEMENT(return {m_ptr->import_model(model, mmap_buffer, context, config), m_so});
}
